...
namespace MobileKit
{
    ...
    public class Pool
    {
        ...
        private GameObject GetPooledObjectSingleType(PooledObjectSettings settings)
        {
            ...
            {
                if (settings.UseActiveOnHierarchy ? !t.activeInHierarchy : !t.activeSelf)
                {
                    SetupPooledObject(t, settings);
                    return t;
                }
            }
        ...
        public void ResetParents()
        {
            if (type == PoolType.Single)
            {
                foreach (var t in pooledObjects)
                {
                    t.transform.SetParent(objectsContainer != null ? objectsContainer : PoolManager.Instance.transform);
                }
            }
            else
            {
                foreach (var t in multiPooledObjects)
                {
                    for (int j = 0; j < t.Count; j++)
                    {
                        t[j].transform.SetParent(objectsContainer != null ? objectsContainer : PoolManager.Instance.transform);
                    }
                }
            }
        }
