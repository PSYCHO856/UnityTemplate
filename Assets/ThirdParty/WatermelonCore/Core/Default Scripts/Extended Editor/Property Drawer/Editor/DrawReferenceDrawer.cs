...
namespace Watermelon
{
    ...
    public class DrawReferenceDrawer : UnityEditor.PropertyDrawer
    {
        ...
        private readonly float m_Space = EditorGUIUtility.standardVerticalSpacing * 2;
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            {
                property.isExpanded =
                    EditorGUI.Foldout(new Rect(position.x, position.y, EditorGUIUtility.labelWidth, position.height),
                        property.isExpanded, label, true);
                EditorGUI.PropertyField(
                    new Rect(position.x + EditorGUIUtility.labelWidth, position.y,
                        position.width - EditorGUIUtility.labelWidth, position.height), property, GUIContent.none);
                ...
                if (property.isExpanded)
                {
                    ...
                    var subIndentLevel = EditorGUI.indentLevel;
        ...
        public override float GetPropertyHeight(SerializedProperty property, GUIContent label)
        {
            ...
            var height = base.GetPropertyHeight(property, label) + m_Space;
            if (m_SerializedObject != null)
                if (property.isExpanded)
                {
                    var prop = m_SerializedObject.GetIterator();
