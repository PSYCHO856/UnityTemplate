...
namespace Watermelon
{
    public class Ease
    {
            ...
            switch (ease)
            {
                ...
                case Type.ElastinInOut:
                    if (p < 0.5f)
                        ...
                        return 0.5f * (Mathf.Sin(-13 * HALF_PI * (2 * p - 1 + 1)) * Mathf.Pow(2, -10 * (2 * p - 1)) +
                                       2);
        ...
        public static EaseFunction GetFunction(Type ease)
        {
            switch (ease)
            {
                case Type.Linear: return p => { return p; };
                case Type.QuadIn: return p => { return p * p; };
                case Type.QuadOut: return p => { return -(p * (p - 2)); };
                case Type.QuadOutIn:
                    return p =>
                    {
                        if (p < 0.5f)
                ...
                case Type.CubicIn: return p => { return p * p * p; };
                case Type.CubicOut:
                    return p =>
                    {
                        var f1 = p - 1;
                ...
                case Type.QuartIn: return p => { return p * p * p * p; };
                case Type.QuartOut:
                    return p =>
                    {
                        var f3 = p - 1;
                ...
                case Type.QuintIn: return p => { return p * p * p * p * p; };
                case Type.QuintOut:
                    return p =>
                    {
                        var f = p - 1;
                ...
                case Type.SineIn: return p => { return Mathf.Sin((p - 1) * HALF_PI) + 1; };
                case Type.SineOut: return p => { return Mathf.Sin(p * HALF_PI); };
                case Type.SineInOut: return p => { return 0.5f * (1 - Mathf.Cos(p * PI)); };
                case Type.CircIn: return p => { return 1 - Mathf.Sqrt(1 - p * p); };
                case Type.CircOut: return p => { return Mathf.Sqrt((2 - p) * p); };
                case Type.CircInOut:
                    return p =>
                    {
                        if (p < 0.5f)
                ...
                case Type.ExpoIn: return p => { return p == 0.0f ? p : Mathf.Pow(2, 10 * (p - 1)); };
                case Type.ExpoOut: return p => { return p == 1.0f ? p : 1 - Mathf.Pow(2, -10 * p); };
                case Type.ExpoInOut:
                    return p =>
                    {
                        if (p == 0.0 || p == 1.0) return p;
                ...
                case Type.ElasticIn: return p => { return Mathf.Sin(13 * HALF_PI * p) * Mathf.Pow(2, 10 * (p - 1)); };
                case Type.ElasticOut:
                    return p => { return Mathf.Sin(-13 * HALF_PI * (p + 1)) * Mathf.Pow(2, -10 * p) + 1; };
                case Type.ElastinInOut:
                    return p =>
                    {
                        if (p < 0.5f)
                ...
                case Type.BackIn: return p => { return p * p * p - p * Mathf.Sin(p * PI); };
                case Type.BackOut:
                    return p =>
                    {
                        var f = 1 - p;
                ...
                case Type.BounceIn: return p => { return BounceEaseIn(p); };
                case Type.BounceOut: return p => { return BounceEaseOut(p); };
                case Type.BounceInOut: return p => { return BounceEaseInOut(p); };
                default: return p => { return p; };
            }
        }
