...
namespace Watermelon
{
    ...
    public class DefinesSettingsEditor : WatermelonEditor
    {
        ...
        private static IEnumerator RecheckDefines()
        {
            ...
            {
                var definesData = EditorUtils.GetAsset<DefinesSettings>();

                if (definesData == null)
                {
                    if (EditorUtility.DisplayDialog("Define Manager", "Defines asset can't be found.", "Create",
                        "Ignore"))
                        EditorUtils.CreateAsset<DefinesSettings>("Assets/Content/Settings/Editor/Define Settings",
                            true);
                    else
        ...
        {
            addDefineButton = new GUIContent(EditorStylesExtended.ICON_SPACE + "Add Define",
        ...
        private string[] GetActiveStaticDefines()
        {
            ...
            if (!string.IsNullOrEmpty(definesLine))
            {
                ...
                var defines = definesLine.Split(';');

                for (var i = 0; i < DefinesSettings.STATIC_DEFINES.Length; i++)
        ...
        private void CacheVariables()
        {
            ...
            // Get static defines
            var activeStaticDefines = GetActiveStaticDefines();
            if (!activeStaticDefines.IsNullOrEmpty())
                for (var i = 0; i < activeStaticDefines.Length; i++)
            ...
            {
                //Get attribute
                var defineAttributes = (DefineAttribute[]) Attribute.GetCustomAttributes(type, typeof(DefineAttribute));

                for (var i = 0; i < defineAttributes.Length; i++)
                {
                    var methodId = defines.FindIndex(x => x.define == defineAttributes[i].define);
                    if (methodId == -1) defines.Add(new Define(defineAttributes[i].define, Define.Type.Project));
                }
            }

            // Get custom defines
            var customDefinesArraySize = customDefinesProperty.arraySize;
            if (customDefinesArraySize > 0)
                for (var i = 0; i < customDefinesArraySize; i++)
                    defines.Add(new Define(customDefinesProperty.GetArrayElementAtIndex(i).stringValue,
                        Define.Type.Custom));

            var defineLine =
                PlayerSettings.GetScriptingDefineSymbolsForGroup(
                    BuildPipeline.GetBuildTargetGroup(EditorUserBuildSettings.activeBuildTarget));
            var currentDefinesArray = defineLine.Split(';');
            for (var i = 0; i < currentDefinesArray.Length; i++)
        ...
        private void LoadActiveDefines()
        {
            ...
            if (!currentDefinesArray.IsNullOrEmpty())
                for (var i = 0; i < currentDefinesArray.Length; i++)
                {
                    var defineIndex = Array.FindIndex(projectDefines, x => x.define.Equals(currentDefinesArray[i]));

                    if (defineIndex != -1) projectDefines[defineIndex].isEnabled = true;
                }
        }
        ...
        private string GetActiveDefinesLine()
        {
            ...
            for (var i = 0; i < projectDefines.Length; i++)
        ...
        private void SaveDefines(string definesLine)
        {
            PlayerSettings.SetScriptingDefineSymbolsForGroup(
                BuildPipeline.GetBuildTargetGroup(EditorUserBuildSettings.activeBuildTarget), definesLine);
        }

        private bool CompareDefines()
        {
            var currentDefinesArray = PlayerSettings
                .GetScriptingDefineSymbolsForGroup(
                    BuildPipeline.GetBuildTargetGroup(EditorUserBuildSettings.activeBuildTarget)).Split(';');

            for (var i = 0; i < projectDefines.Length; i++)
            {
                var findIndex = Array.FindIndex(currentDefinesArray, x => x == projectDefines[i].define);
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (!projectDefines.IsNullOrEmpty())
            {
                ...
                for (var i = 0; i < projectDefines.Length; i++)
                {
                    ...
                    switch (projectDefines[i].type)
                    {
                        ...
                        case Define.Type.Project:
                            projectDefines[i].isEnabled =
                                EditorGUILayout.Toggle(projectDefines[i].isEnabled, GUILayout.Width(20));
                            EditorGUILayout.PrefixLabel(projectDefines[i].define);
                        ...
                        case Define.Type.Custom:
                            ...
                            if (GUILayout.Button("X", EditorStylesExtended.button_04_mini, GUILayout.Height(18),
                                ...
                                if (EditorUtility.DisplayDialog("Remove define",
                                    "Are you sure you want to remove define?", "Remove", "Cancel"))
                                {
                                    var isRemovedDefineEnabled = projectDefines[i].isEnabled;

                                    customDefinesProperty.RemoveFromVariableArrayAt(customDefineIndex);

                                    CacheVariables();

                                    if (isRemovedDefineEnabled) SaveDefines(GetActiveDefinesLine());
                        ...
                        case Define.Type.ThirdParty:
                            ...
                            if (GUILayout.Button("X", EditorStylesExtended.button_04_mini, GUILayout.Height(18),
                                ...
                                if (EditorUtility.DisplayDialog("Remove define",
                                    "Are you sure you want to remove define?", "Remove", "Cancel"))
                                {
                                    var defineLine = PlayerSettings.GetScriptingDefineSymbolsForGroup(
                                        BuildPipeline.GetBuildTargetGroup(EditorUserBuildSettings.activeBuildTarget));
                                    var currentDefinesArray = defineLine.Split(';');

                                    defineLine = "";
                                    for (var k = 0; k < currentDefinesArray.Length; k++)
                ...
                if (EditorGUI.EndChangeCheck()) isRequireInit = true;
            }
            else
            {
            ...
            if (tempDefine != null)
            {
                ...
                if (GUILayout.Button("✓", EditorStylesExtended.button_03_mini, GUILayout.Height(18),
                    GUILayout.Width(18)))
                {
                    if (Array.FindIndex(projectDefines, x => x.define == tempDefine.define) != -1)
                    {
                        EditorUtility.DisplayDialog("Wrong Define", tempDefine.define + " define already exists!",
                            "Close");
                    ...
                    var index = customDefinesProperty.arraySize;

                    customDefinesProperty.arraySize++;

                    var newElementProperty = customDefinesProperty.GetArrayElementAtIndex(index);
            ...
            var compileRect = EditorGUILayout.BeginVertical(EditorStylesExtended.editorSkin.box);
